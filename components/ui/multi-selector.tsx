...
import "./multi-selector.component.css";
...
const MultiSelector = forwardRef<MultiSelectRef, MultiSelectorProps>(
  (
    {
      values: value,
...
type SelectItem = { value: string; label: string; isNew?: boolean };

export type MultiSelectorMapProps = React.ComponentProps<
...
const CreatableItem = ({ items }: Pick<MultiSelectorMapProps, "items">) => {
  const {
    inputValue,
    setInputValue,
    onValueChange,
  ...
  const isItemExist = useCallback(
    (items: SelectItem[], newItem: SelectItem[]) => {
      return items.some((item) => item.value === newItem[0].value);
    },
    [],
  );

  if (
    isItemExist(items, [{ value: inputValue, label: inputValue }]) ||
    values.find((s) => s === inputValue)
  ) {
    return undefined;
  }
...
const MultiSelectorMap = forwardRef<MultiSelectRef, MultiSelectorMapProps>(
  ({ items, placeholder, className, creatable, ...props }, ref) => {
    return (
      <MultiSelector {...props} ref={ref}>
        <MultiSelectorTrigger className={className}>
          <MultiSelectorInput placeholder={placeholder} />
        </MultiSelectorTrigger>
        <MultiSelectorContent>
          {creatable && <CreatableItem items={items} />}
          {items.map((item) => (
